  ...
  return (
    <div className='topic-article-content topic-social'>
      <div className='participants-box'>
        <span className='paticipants-box-published'>Published</span>
        <span>{
          moment(topic.publishedAt).format('MMMM Do, YYYY')
        }</span>
        <span className={ topic.closed ? 'icon-lock' : 'icon-lock-open'} style={{marginRight: '5px'}}></span>
        <span className={ topic.closed ? 'Closed' : 'Available'} >{topic.closed ? 'Closed' : 'Open'}</span>
