...
export default class HomeForum extends Component {
  ...
  render () {
    ...
    const tipoConsulta = forum.extra.contentType === 'llamado' && 'convocatoria' || 'consultation'
      ...
      <div className='ext-forum-home'>
            ...
            <div className='container'>
              ...
              { forum.extra.contentType === 'propuestas' && 
                <a
                  className='btn btn-primary'
                  onClick={this.handleScroll} >
                  Choose a topic and participate
                </a>
              }
            ...
            <div className='row'>
              <div className='col-md-8 offset-md-2'>
                <div className="consulta-cerrada">
                  ...
                  <h5>The {tipoConsulta} has ended</h5>
                  ...
                  <p>The {tipoConsulta} is no longer open to comments or participation.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className='container topics-container' >
          {this.state.topics.length > 0 && (forum.extra.contentType === 'ejes' || forum.extra.contentType === undefined) &&
            <h5 id='anchor'>{`${this.state.topics.length} Axis make this proposal`}</h5>
          }
          {this.state.topics.length > 0 && forum.extra.contentType === 'propuestas' &&
            <h5 id='anchor'>{`${this.state.topics.length} ${this.state.topics.length > 1 ? 'proposals' : 'proposal'} in this request`}</h5>
          }
          {this.state.topics.length > 0 && forum.extra.contentType === 'llamado' &&
            <h5 id={!isClosed && forum.extra.contentType === 'llamado' ? '' : 'anchor'}> {this.state.topics.length} topics are included in this consultation</h5>
          }
